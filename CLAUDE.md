...
Project Context: Nabu - AI-Powered Feedback Intelligence

Build a production-grade feedback aggregation system that demonstrates Cloudflare's serverless AI capabilities through a hybrid classification approach.

Core Requirements

1. Hybrid AI Classification Engine
   - Use `@cf/baai/bge-base-en-v1.5` for semantic embedding-based classification (95% of workload)
   - Use `@cf/meta/llama-3.1-8b-instruct` for executive summary generation (5% of workload)
   - Implement dynamic confidence thresholding (50% for "Critical" urgency)
   - Store raw classification scores in D1 to enable threshold adjustments without re-processing

2. Multi-Dimensional Classification
   Each feedback item must be classified across:
   - Product/Service: D1, Workers AI, KV, R2, Workflows, Pages, Stream, etc.
   - Sentiment: Positive, Neutral, Negative
   - Urgency: Critical, High, Medium, Low (with confidence scoring)
   - Feedback Type: Bug, Feature Request, Documentation, Performance, Pricing
   - Churn Risk (optional): At-risk, Satisfied, Neutral

3. Data Architecture
   - Use D1 for relational storage with complex aggregations
   - Implement batch processing (50 items per chunk) to respect API limits
   - Use `db.batch()` for parallel query execution to minimize latency
   - Support 1000+ feedback items with timestamps distributed across 365 days

4. Dashboard Requirements
   - Vanilla JS + Tailwind CSS hosted via Cloudflare Assets
   - Real-time charts: Volume Trends, Sentiment Distribution, Theme Analysis
   - Client-side keyword search with 300ms debounce
   - Server-side filtering: Source, Sentiment, Status, Date Range, Importance
   - Importance-based sorting using SQL CASE statements
   - Executive Summary panel with LLM-generated insights

5. API Design Patterns
   - Implement conditional summary logic:
     - Prioritize 7-day urgent issues if available
     - Fallback to 30-day critical issues if < 5 important 7-day items
     - Fallback to year-long trends if < 5 critical 30-day items
   - Include excerpts from the last 5 critical feedback items in LLM prompts
   - Support pagination with `limit` and `offset` parameters

6. Performance Optimizations
   - Batch all SQL queries using `env.DB.batch()` for metrics endpoints
   - Use `Promise.all()` for parallel API calls in the frontend
   - Implement frontend debouncing for search inputs
   - Minimize LLM token usage by pre-aggregating data

7. Developer Experience Considerations
   - Log friction points with Cloudflare products (Workers AI, D1, Wrangler)
   - Document model discovery issues (e.g., `llms.txt` gaps)
   - Track deployment speed and CLI interactivity blockers
   - Note query optimization patterns for D1 performance

Technical Constraints

- No external dependencies beyond Cloudflare's native bindings
- Edge-native architecture: All compute and AI inference at the edge
- Cost-efficiency: Prioritize embeddings over LLMs for bulk classification
- Type safety: Use TypeScript with proper interface definitions
- SQL-first analytics: Leverage D1's relational capabilities for aggregations

Example Use Cases

- Aggregate feedback from Discord, GitHub, Twitter, and Support channels
- Automatically classify and prioritize 1100+ items in < 10 seconds
- Generate executive summaries that adapt to data density
- Enable PMs to filter by product, sentiment, and urgency in real-time
- Track resolution status and formal issue escalations
